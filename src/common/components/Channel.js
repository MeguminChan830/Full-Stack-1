...
export default class Channels extends Component {
    ...
    validateChannelName() {
        ...
        if (channels.filter(channel => channel.name === this.state.channelName.trim()).length > 0) {
            return "error"
        }
        return "success"
    }
    ...
    render() {
        ...
        const moreChannelsBoolean = channels.length > 0
                    ...
                    <Modal.Body>
                        <form onSubmit={this.handleModalSubmit}>
                            <Input
                                ref="channelName"
                                type='text'
                                help={this.validateChannelName() === "error" && "A Channel with thats name already exists!"}
                                bsStyle={this.validateChannelName()}
                                hasFeedback
                                name="channelName"
                                autoFocus="true"
                                placeHolder='Enter tha channel name'
                                valur={this.state.channelName}
                                onChange={this.handleCModalChange} />
                        </form>
                    </Modal.Body>
                    <Modal.Footer>
        ...
        const moreChannelsModal = (
            <div style={{ background: 'grey' }}>
                <Modal key={2} show={this.state.moreChannelsModal} onHide={this.closeMoreChannelModal}>
                    <Modal.Header closeButton>
                    ...
                    <Modal.Body>
                        <ul style={{ style: 'auto', margin: '0', overflowY: 'auto', padding: '0' }}>
            ...
            <section>
                <div>
                    <span style={{paddingLeft: '0.8em', fontSize: '1.5em'}}>
                        Channels
                        <button onClick={this.openAddChannnelModal} style={{fontSize: '0.8em', "background": "Transparent", marginLeft: '2.8em', "backgroundRepeat":"noRepeat", 'border': 'none', "cursor": "pointer", "overflow": 'hidden', "outline": 'noone' }}>
                ...
                <div>
                    <ul style={{display: 'flex', flexDirection: "column", listStyle: 'none', margin: '0', overflowY: 'auto', padding: '0'}}>
                        {filteredChannels.map(channel=>
                            <ChannelListItem style={{paddingLeft: '0.8em', background: '#2e6da4', height: '0.7em'}} messageCount={messages.filter(msg=>msg.channelID===channel.name).length} channel={channel} key={channel.id} onClick={this.handleChangeChannel}/>
